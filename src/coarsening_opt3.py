##Method3: Change the encoding layer to a 2-layer graphsage

import torch
import torch.nn.functional as F
from torch.nn import Linear,Sequential, ReLU, BatchNorm1d as BN

from torch_geometric.nn import DenseSAGEConv, DenseGCNConv, JumpingKnowledge, DenseGINConv
from .Sinkhorn import sinkhorn_loss_default

class GNNBlock(torch.nn.Module): #2 layer GraphSAGE block
...
class MultiLayerCoarsening(torch.nn.Module):
    def __init__(self, dataset, hidden, num_layers=2, ratio=0.5):
        ...
        # self.embed_block1 = DenseGINConv(Sequential(
        #     Linear(dataset.num_features, hidden),
        #     ReLU(),
        #     Linear(hidden, hidden),
        #     ReLU(),
        #     BN(hidden)),train_eps=True)
